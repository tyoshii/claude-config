...
報告の後、`.claude/rules/` 配下にサプライチェーン保護に関するルールが存在するか確認する。

1. `.claude/rules/` 配下のファイルを Glob で一覧し、各ファイルの内容を読み取る
2. 以下のようなサプライチェーン保護に関する記述があるか確認する：
   - Actions のコミット SHA ピン留めに関するルール
   - npm パッケージ追加時の公開日チェックに関するルール
   - install スクリプトの確認に関するルール
3. **該当するルールが見つからない場合**、ユーザーに以下のように提案する：

> `.claude/rules/` にサプライチェーン保護に関するルールが見つかりませんでした。
> 以下のようなルールを追加しますか？
>
> - GitHub Actions では必ずコミット SHA でバージョンをピン留めし、タグ・ブランチ参照を使わない
> - npm パッケージを新規追加する際は、公開から 3 日未満のバージョンを避ける
> - `preinstall` / `postinstall` スクリプトを持つパッケージは導入前に内容を確認する

ユーザーが承認した場合のみ、`.claude/rules/` にルールファイルを作成する。ファイル名は `supply-chain.md` とする。
