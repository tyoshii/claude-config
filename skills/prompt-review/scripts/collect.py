...
def _set_period(result: dict, messages: list[dict]) -> None:
    """messages リストから期間（period）を算出して result に設定する"""
...
def collect_copilot_chat(cutoff_ms: int | None, project_filter: str | None) -> dict:
    ...
    if all_prompts:
        ...
        _set_period(result, all_prompts)
...
def _collect_cline_like(tool_name: str, extension_id: str, cutoff_ms: int | None) -> dict:
    """Cline / Roo Code 共通の収集ロジック（同一構造の api_conversation_history.json を読む）"""
    result = {"tool": tool_name, "status": "未検出", "messages": [], "period": ""}

    appdata = get_appdata_path()
    tasks_dir = appdata / "Code" / "User" / "globalStorage" / extension_id / "tasks"
...
def collect_cline(cutoff_ms: int | None) -> dict:
    ...
    return _collect_cline_like("Cline", "saoudrizwan.claude-dev", cutoff_ms)


def collect_roo_code(cutoff_ms: int | None) -> dict:
    """Roo Code の会話履歴を収集（Clineと同じ構造）"""
    return _collect_cline_like("Roo Code", "RooVeterinaryInc.roo-cline", cutoff_ms)
...
def collect_windsurf(cutoff_ms: int | None) -> dict:
    ...
    if all_entries:
        ...
        _set_period(result, all_entries)
...
def collect_gemini_cli(cutoff_ms: int | None, project_filter: str | None) -> dict:
    ...
    if all_messages:
        ...
        _set_period(result, all_messages)
...
def collect_opencode(cutoff_ms: int | None, project_filter: str | None) -> dict:
    ...
    for db_path in db_paths:
        ...
        try:
            ...
            query += " ORDER BY m.time_created ASC, pt.time_created ASC, pt.id ASC LIMIT 5000"
